...
class Simulation:
    ...
    def _load_boundary_data(self, smv_file: TextIO, line: str, cell_centered: bool):
        ...
        with open(file_path, 'rb') as infile:
            ...
            # infile_size = 3 * 38 + 12 + (n_patches * 44) + (n_t * (12 + sum_qq_bytes))
            sum_qq_bytes = 0

            for i in range(n_patches):
                i1, i2, j1, j2, k1, k2, ior, nb, nm = patch_infos[i][0]
                count = (i2 - i1 + 1) * (j2 - j1 + 1)
                sum_qq_bytes += 4 + count + 4
                print(f'sum_qq_bytes: {sum_qq_bytes}')
            print('ended the for loop for sum_qq_bytes')
            n_t = (infile_size - 3 * 38 - 12 - (n_patches * 44)) / (12 + sum_qq_bytes)
            # n_t = 1001
